...
describe("Testing the update uuCmd...", () => {
  ...
  test("extra parameter (warning)", async () => {
    ...
    expect(result.status).toEqual(200);
    expect(result.visibility).toEqual(dtoIn.visibility);
    expect(result.data.uuAppErrorMap).toMatchObject({"uu-belist-main/joke/update/unsupportedKeys": {"message": "DtoIn contains unsupported keys.", "paramMap": {"unsupportedKeyList": ["$.extra"]}, "type": "warning"}});
  });
